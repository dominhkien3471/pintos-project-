...
// static void *arg_pass(void * esp, char *cmd, char *save_ptr);
...
/*  Argument passing.
    Stack có cấu trúc như sau:
esp ->      +    argv[n]        +  Giá trị của argv
            +    argv[n -1]     +
            +++++++++++++++++++++               
            +    argv[0]        +
            +    word-align     +  word-align có nhiệm vụ padding vào để sao cho bắt đầu một ô nhớ đều chia hết cho 4
            +    argv[n]        +  Địa chỉ của argv
            +     ++++          +
            +     argv          +
            +     argc          +
            +  return address   +
...
/* static void *arg_pass(void* esp, char *cmd, char *save_ptr)
{
    ...
    // Push argc 
    esp -= sizeof(uint32_t);
    memset(esp, i, sizeof(uint32_t));

    // Write NULL pointer to return;
    esp -= sizeof(void*);
    esp = NULL;
